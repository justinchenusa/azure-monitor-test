...
public class CheckFunctionAppMetrics {
	public static boolean runMetrics02(Azure azure) {		
		...
		try {
			...
			AppServiceManager appServiceMgr = azure.appServices();
			
			PagedList<FunctionApp> functionApps = appServiceMgr.functionApps().list();
			
			for (FunctionApp functionApp : functionApps) {
				System.out.println("Function App AvailablityState: " + functionApp.availabilityState().name());
				List<MetricDefinition> metricDefinitions = azure.metricDefinitions().listByResource(functionApp.id());
				for (MetricDefinition metricDefinition : metricDefinitions) {		
			        ...
			        VMMetricsUtils.printMetricCollection(metricCollection, functionApp.id());
				}
